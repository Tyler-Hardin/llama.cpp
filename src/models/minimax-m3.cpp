...
void llama_model_minimax_m3::load_arch_hparams(llama_model_loader & ml) {
    ...
    const bool has_indexer = ml.get_key(LLM_KV_ATTENTION_INDEXER_HEAD_COUNT,    hparams.indexer_n_head,       false);
...
void llama_model_minimax_m3::load_arch_tensors(llama_model_loader &) {
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        if (i < (int) hparams.n_layer_dense_lead) {
            ...
            // indexer (only when GGUF has MSA indexer parameters)
...
llama_model_minimax_m3::graph::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    if (msa_enabled && hparams.indexer_kv) {
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            const bool is_sparse = msa_enabled && il >= (int) hparams.n_layer_dense_lead && hparams.indexer_kv;
