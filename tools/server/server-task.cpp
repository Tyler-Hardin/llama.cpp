...
json task_params::to_json(bool only_metrics) const {
    ...
    if (only_metrics) {
        return json {
            ...
            {"samplers",                  samplers},
            {"speculative.type",          common_speculative_type_name_str(speculative.types)},
...
task_params server_task::params_from_json_cmpl(
    ...
    params.speculative.type = common_speculative_type_from_name(json_value(data, "speculative.types", common_speculative_type_to_str(defaults.speculative.type)));
    ...
    if (data.contains("json_schema") && !data.contains("grammar")) {
        ...
        // Pre-trigger (thinking-phase) grammar: applied only inside reasoning blocks.
        // When both a user grammar and tools are present, the server routes the
        // user grammar here so it constrains the thinking output while leaving
        // the tool-call grammar free to handle tool calls after </think>.
        std::string thinking_grammar_str = json_value(data, "thinking_grammar", std::string());
        if (!thinking_grammar_str.empty()) {
            params.sampling.pre_trigger_grammar = {COMMON_GRAMMAR_TYPE_USER, std::move(thinking_grammar_str)};
        } else {
            // Carry forward the server-wide thinking grammar default from the
            // CLI / --thinking-grammar-file flag.
            params.sampling.pre_trigger_grammar = params_base.sampling.pre_trigger_grammar;
        }
    }
...
server_prompt_cache_state * server_prompt_cache::alloc(const server_prompt & prompt, size_t state_size_main, size_t state_size_drft) {
    ...
    const size_t state_size_new = state_size_main + state_size_drft + checkpoints_size;
    ...
    try {
        state_data_tgt.resize(state_size_main);
        state_data_dft.resize(state_size_drft);
